...
# Load the CSV file
file_path = "motor2_updated_ankle_current_static250407_053556.csv"
...
print(f"Coulomb Friction (f_c): {fc:.6f} Nm")
print(f"Gear Friction (f_g): {fg:.6f} Nm/A")
print(f"Damping Coefficient (B_a): {Ba:.6f} Nm.s/rad")
print(f"Moment of Inertia (J_a): {Ja:.6f} Nm.s²/rad")
...
plt.scatter(I, τ_predicted, label="Predicted Data", color="red", alpha=0.7)
plt.xlabel("Motor Current (A)")
plt.ylabel("Torque (Nm)")
plt.title("Torque vs Current (Regression Model for filtered data)")
plt.legend()
plt.grid()
plt.show()
...
plt.scatter(index, I, label="Motor Current (A)", color="blue", alpha=0.7, s=10)
plt.scatter(index, τ, label="1:1 gear ratio Torque (τ, Nm)", color="green", alpha=0.5, s=10, edgecolors='black')
plt.scatter(index, torque, label="9:1 gear ratio Torque (τ, Nm)", color="red", alpha=0.3, s=10, edgecolors='none')

plt.xlabel("index")
plt.ylabel("Values")
plt.title("Motor Current and Torque over Time")
plt.legend()
plt.grid(True)
plt.tight_layout()
plt.show()
...
# plt.figure(figsize=(10, 6))
# plt.scatter(index, τ, label="Measured Data", color="blue", alpha=0.5, s=10, edgecolors='black')
# plt.xlabel("index ")
# plt.ylabel("Torque (Nm)")
# plt.title("Torque vs index")
# plt.legend()
# plt.grid()
# plt.show()
